...
router.get('/:page',(req,res)=>{
    ...
    if(req.session.count){
        client.query('SELECT * FROM Sales,Menu_All_List WHERE Sales.Menu_Num=Menu_All_List.Menu_Num',(error,results)=>{
            if(error){
                res.send('<h1>'+'Database ERROR!!!'+'</h1>');
            }else{
                fs.readFile('sellerinquiretakeofmenu.jade','utf8',(error,data)=>{
                    if(error){
